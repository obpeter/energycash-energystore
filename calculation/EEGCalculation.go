...
func calculateReport(iter ValueIterator,
	...
	var err error
	lastLineId := "" // ToDo: Rethink Report ID generation. Or consider a new iteration strategy
	var _line model.RawSourceLine
	for iter.Next(&_line) {
		line := _line.Copy(defaultConsumerLen)

		if switchIntermediate(line.Id) {
			reportSeries = append(reportSeries, &model.EnergyReport{
				Id:            intermediateId(lastLineId),
	...
	return reportSeries, &model.EnergyReport{
		Id:            reportId,
...
func CalculateMonthlyPeriod(db *store.BowStorage, allocFunc AllocationHandlerV2, year, month int) ([]*model.EnergyReport, *model.EnergyReport, error) {
	...
	cDay := 0
	return calculateReport(iter, metaInfo, allocFunc, fmt.Sprintf("YM/%d/%.2d", year, month),
		...
		func(rowId string) string {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			return fmt.Sprintf("IRP/%d/%.2d/%.2d", year, month, lineTime.Day())
		},
	)
}
...
func CalculateAnnualPeriod(db *store.BowStorage, allocFunc AllocationHandlerV2, year int) ([]*model.EnergyReport, *model.EnergyReport, error) {
	...
	cMonth := 0
	return calculateReport(iter, metaInfo, allocFunc, fmt.Sprintf("Y/%d/0", year),
		func(rowId string) bool {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			if cMonth == 0 {
				cMonth = int(lineTime.Month())
				return false
			}
			shouldSwitch := int(lineTime.Month()) > cMonth
			cMonth = int(lineTime.Month())
			return shouldSwitch
		},
		func(rowId string) string {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			return fmt.Sprintf("IRP/%d/0/%.2d/", year, lineTime.Month())
		},
	)
}
...
func CalculateBiAnnualPeriod(db *store.BowStorage, allocFunc AllocationHandlerV2, year, segment int) ([]*model.EnergyReport, *model.EnergyReport, error) {
	...
	cMonth := 0
	return calculateReport(iter, metaInfo, allocFunc, fmt.Sprintf("YH/%d/%d", year, segment),
		func(rowId string) bool {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			if cMonth == 0 {
				cMonth = int(lineTime.Month())
		...
		func(rowId string) string {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			return fmt.Sprintf("IRP/%d/%.2d/%.2d", year, segment, lineTime.Month())
		},
	)
}
...
func CalculateQuarterlyPeriod(db *store.BowStorage, allocFunc AllocationHandlerV2, year, segment int) ([]*model.EnergyReport, *model.EnergyReport, error) {
	...
	cMonth := 0
	return calculateReport(iter, metaInfo, allocFunc, fmt.Sprintf("YQ/%d/%d", year, segment),
		func(rowId string) bool {
			lineTime, _ := utils.ConvertRowIdToTime(rowPrefix, rowId)
			if cMonth == 0 {
				cMonth = int(lineTime.Month())
...
//
//		annual_ps += ps
//		glog.V(4).Infof("AllocMonth (%d) %f - %+v", m, ps, am)
//	}
//
